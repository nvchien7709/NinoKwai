...
module.exports.run = async function({ api, event, args, Currencies }) {
  ...
  if (!args[0] || !isNaN(args[0])) return api.sendMessage("[ Bầu Cua ] - Hãy Bấm: ?baucua [bầu/cua/cá/nai/gà/tôm] + [số tiền]", event.threadID, event.messageID);
  ...
  switch (args[0]) {
    ...
    case "Tôm": itemm = "tom";
      icon = '🦞';
      break;
    default: return api.sendMessage("[ Bầu Cua ] - Hãy Bấm: /baucua [bầu/cua/cá/nai/gà/tôm] [số tiền]", event.threadID, event.messageID);
  }
  await get(number[0], number[1], number[2]);
  api.sendMessage({ body: "[ Bầu Cua ] - Đang lắc...", attachment: createReadStream(__dirname + "/baucua/baucua.gif") }, event.threadID, async (error, info) => {
    ...
    if (array.includes(itemm)) {
      ...
      if (i == 1) {
        ...
        return api.sendMessage({ body: `== [ Kết Quả Bầu Cua ] ==\n            ‹ ${full.join("|")} ›\nĐược ${mon} vnđ,Vì Có 1 ${icon}!`, attachment: listimg }, event.threadID, event.messageID);
      }
      else if (i == 2) {
        var mon = parseInt(args[1]) * 2;
        await Currencies.increaseMoney(event.senderID, mon); console.log("s2")
        return api.sendMessage({ body: `== [ Kết Quả Bầu Cua ] ==\n            ‹ ${full.join("|")} ›\nĐược ${mon} vnđ,Vì Có 2 ${icon}!`, attachment: listimg }, event.threadID, event.messageID);
      }
      else if (i == 3) {
        var mon = parseInt(args[1]) * 3;
        await Currencies.increaseMoney(event.senderID, mon); console.log('s3')
        return api.sendMessage({ body: `== [ Kết Quả Bầu Cua ] ==\n            ‹ ${full.join("|")} ›\nĐược ${mon} vnđ,Vì Có 3 ${icon}!`, attachment: listimg }, event.threadID, event.messageID);
      }
      ...
      return api.sendMessage({ body: `== [ Kết Quả Bầu Cua ] ==\n            ‹ ${full.join("|")} ›\nTrừ ${args[1]} vnđ,Vì Có 0 ${icon}!`, attachment: listimg }, event.threadID, event.messageID);
    }
  }, event.messageID);
};